...
for i, nsamples in enumerate(nsampless):
    for j, nsteps in enumerate(nstepss):
        data = np.load(f'./profiling_time/results/times-diffusion-{nsamples}-{nsteps}.npy')
        container_diff[i, j] = np.mean(data)

for i, nsamples in enumerate(nsampless):
    for j, eps in enumerate(epss):
        data = np.load(f'./profiling_time/results/times-ot-{nsamples}-{eps}.npy')
...
if style == '3d':
    ...
    for i, eps in enumerate(epss):
        ax.plot(np.log2(nsampless), np.log2(eps * 80 * np.ones(len(nsampless))), container_ot[:, i], c='tab:blue')
