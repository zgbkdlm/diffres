...
parser.add_argument('--nsteps', type=int, default=32, help='The integration steps.')
parser.add_argument('--integrator', type=str, default='euler', help='The integrator.')
parser.add_argument('--sde', action='store_true', help='The flow model.')
...
@jax.jit
def resampling():
    return diffusion_resampling(key, log_ws, prior_samples, a, ts, integrator=integrator, ode=ode)
...
# Save result
print(f'diffres (id={args.mc_id}) with a={a} | T={T} | nsteps={nsteps} | {integrator} {"| ode" if ode else "| sde"} '
      f'has err {err} and time {time}.')
np.savez_compressed(f'./gms/results/diffres-{a}-{T}-{nsteps}-{integrator}-{"ode" if ode else "sde"}-{args.mc_id}.npz',
