...
@jax.jit
def resampling():
    return gumbel_softmax(key, log_ws, prior_samples, tau)
...
# Save result
print(f'Gumbel-softmax (id={args.mc_id}) with tau={tau} has err {err} and time {time}.')
np.savez_compressed(f'./gms/results/gumbel-{tau}-{args.mc_id}.npz',
