...
parser.add_argument('--eps', type=float, default=0.2, help='The OT regulariser.')
...
for mc_id, key_mc in zip(np.arange(args.id_l, args.id_u + 1), keys_mc):
    ...
    def resampling(key_, log_ws_, prior_samples_):
        return ensemble_ot(key_, log_ws_, prior_samples_, eps)
    ...
    # Save result
    print(f'OT (id={mc_id}) with eps={eps} has err {err}.')
    np.savez_compressed(f'./gms/results/ot-{eps}-{mc_id}.npz',
