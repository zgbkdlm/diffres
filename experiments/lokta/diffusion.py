...
prefix = f'Diffres ({mc_id}) | a {a} | T {T} | dsteps={dsteps} | {integrator} {"| ode" if ode else "| sde"}'
losses = np.zeros(args.niters)
for i in range(args.niters):
    key, _ = jax.random.split(key)
    loss = train_step(model, optimiser, key)
    losses[i] = loss
    print(prefix + f' | Iter {i} | loss {loss} | target loss {target_nll}')
nnx_save(model, f'./lokta/checkpoints/diffres-{a}-{T}-{dsteps}-{integrator}-{"ode" if ode else "sde"}-{mc_id}')
...
print(prefix + f' | Prediction RMSE {pred_err}')
np.savez_compressed(f'./lokta/results/diffres-{a}-{T}-{dsteps}-{integrator}-{"ode" if ode else "sde"}-'
                    f'{mc_id}.npz',
