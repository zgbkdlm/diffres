...
def ps(key_, params, ys_):
    def logpdf_trans(x_kp1, xks):
        return jnp.sum(jax.scipy.stats.norm.logpdf(x_kp1, params[0] * xks, sig ** 0.5), axis=-1)

    key_f, key_s = jax.random.split(key_, num=2)
    key_s = jax.random.split(key_s, num=nparticles)
    samples_f, log_ws_f, nll, _ = pf(key_f, params, ys_, multinomial)
    smoother = lambda k, s, w: bootstrap_backward_smoother(k, s, w, logpdf_trans)
    smoothing_trajs = jax.vmap(smoother, in_axes=[0, None, None])(key_s, samples_f, log_ws_f)
    return smoothing_trajs, nll


def logpdf_ssm(params_, traj_, ys_):
    logpdf_trans = (jnp.sum(jax.scipy.stats.norm.logpdf(traj_[1:], params_[0] * traj_[:-1], sig ** 0.5))
                    + jnp.sum(jax.scipy.stats.norm.logpdf(traj_[0], 0., 1.)))
    logpdf_obs = jnp.sum(jax.scipy.stats.norm.logpdf(ys_, params_[1] * traj_, xi ** 0.5))
    return logpdf_trans + logpdf_obs


@jax.jit
def reinforce(key_, params_, ys_):
    smoothing_trajs, nll = ps(key_, params_, ys_)
    grads = jax.vmap(jax.grad(logpdf_ssm), in_axes=[None, 0, None])(params_, smoothing_trajs, ys_)
    return nll, -jnp.mean(grads, axis=0)


@jax.jit
def diffpf(key_, params, ys_):
...
for i in range(nmcs):
    print(i)
    loss_dp, grad_dp = diffpf(keys[i], params, ys)
    loss_rein, grad_rein = reinforce(keys[i], params, ys)
    losses_dp[i] = loss_dp
